...
class ConfusionMatrix(object):
    ...
    def update(self, a, b): #a应为预测，b为预测标签
        n = self.num_classes
        if self.mat is None:
            # 创建混淆矩阵
    ...
    def compute(self):
        ...
        #     TP = self.mat[i, i]
        #     FP = torch.sum(self.mat[i, :]) - TP
        #     FN = torch.sum(self.mat[:, i]) - TP
        #     TN = torch.sum(self.mat) - TP - FP - FN
        #     Precision = TP / (TP + FP)if TP + FP != 0 else 0.
        #     Recall = TP / (TP + FN) if TP + FN != 0 else 0.
        #     # Specificity = TN / (TN + FP) if TN + FP != 0 else 0.
        return acc_global, acc, iu, recall
    ...
    def __str__(self):
        acc_global, acc, iu, recall = self.compute()
        return (
            'global correct: {:.1f}\n'
            'average row correct: {}\n'
            'IoU: {}\n'
